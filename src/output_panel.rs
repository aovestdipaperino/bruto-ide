...
use turbo_vision::core::state::{SF_SHADOW, StateFlags};
...
impl View for OutputPanel {
    ...
    fn handle_event(&mut self, e: &mut Event) {
        self.window.handle_event(e);
    }
    fn can_focus(&self) -> bool {
        true
    }
    fn set_focus(&mut self, f: bool) {
        self.window.set_focus(f);
    }
    ...
    fn init_after_add(&mut self) {
        self.window.init_after_add();
    }
    fn constrain_to_parent_bounds(&mut self) {
        self.window.constrain_to_parent_bounds();
    }
